...
import { createContact, type ContactInput } from "@/lib/api"
...
export function ContactSheet({ open, onOpenChange, onSuccess, defaultType = "supplier" }: ContactSheetProps) {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="max-w-xl h-full overflow-y-auto">
        ...
        <div className="grid grid-cols-1 gap-4 p-4 text-sm md:grid-cols-2">
          {mensagem && <div className="col-span-2 text-xs text-red-600">{mensagem}</div>}
          ...
          <div>
            ...
            <select
              className="bg-background h-10 w-full rounded-md border px-2"
              value={form.pessoa}
              onChange={(e) => update("pessoa", e.target.value as FormState["pessoa"])}
            >
