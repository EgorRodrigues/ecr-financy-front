...
export default function ConfiguracoesPage() {
  const [theme, setTheme] = useState<"light" | "dark">(() => {
    try {
      return ((typeof window !== "undefined" && localStorage.getItem("financy_theme")) as "light" | "dark") || "light"
    } catch {
      return "light"
    }
  })
  const [mode, setMode] = useState<"default-2" | "minimal-3">(() => {
    try {
      const cols = typeof window !== "undefined" ? localStorage.getItem("financy_form_cols") : null
      const compact = typeof window !== "undefined" ? localStorage.getItem("financy_form_compact") : null
      ...
      <Card className="p-3">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            ...
            <select className="bg-background h-9 w-full rounded-md border px-2 text-sm" value={theme} onChange={(e) => setTheme(e.target.value as "light" | "dark")}>
          ...
          <div>
            ...
            <select className="bg-background h-9 w-full rounded-md border px-2 text-sm" value={mode} onChange={(e) => setMode(e.target.value as "default-2" | "minimal-3")}>
